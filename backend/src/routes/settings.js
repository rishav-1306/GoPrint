...
router.put('/', authenticate, requireRole('Admin'), async (req, res, next) => {
  try {
    const { company_name, theme_name, default_printer_id, external_api_url, excel_url, timezone, date_format } = req.body;
    const result = await db.query(
      `INSERT INTO application_settings (id, company_name, theme_name, default_printer_id, external_api_url, excel_url, timezone, date_format, last_updated)
      ...
      [company_name || 'RSB Transmissions', theme_name || 'light', default_printer_id || null,
       external_api_url || null, excel_url || null, timezone || 'Asia/Kolkata', date_format || 'DD-MMM-YYYY']
    );
